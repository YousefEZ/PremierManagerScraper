...
class Manager:
    ...
    def create_stats_url(self, page_number: str) -> str:
        ...
        for page_number in range(1, 100):
            ...
            for raw_row in rows:
                ...
                if identifier in managers:
                    print(f"Duplicated Manager for ({self._manager_number}): {self._name}, {identifier}: {row[0].find('a').get('title')} [{page_number}]")
                    #print(f"Completed Manager ({self._manager_number}): {self._name}")
                    continue
                managers[identifier] = { 
...
MANAGER_STATS_FIELDS = ["id", "name", "targetId", "targetName", "matches", "wins", "draws", "losses"]
